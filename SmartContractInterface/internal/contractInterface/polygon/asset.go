...
func InitializeBRLA() {
	var err error
	// Get chain ID
	ChainID, err = Polygon.ChainID(context.Background())
	if err != nil {
		log.Fatal(err.Error())
	}
...
func UpdateNonce() error {
	nonce, err := Polygon.PendingNonceAt(context.Background(), OwnerAddress)
	if err != nil {
		return err
	}
...
func GetWalletBalance(tokenAddress, walletAddress string) (string, error) {
	...
	res, err := Polygon.CallContract(context.Background(), msg, nil)
	if err != nil {
		return "", err
	}
...
func setupTransaction(value, gasLimit int64) (*bind.TransactOpts, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("[Polygon.setupTransaction] internal error: %w", err)
	}
...
func getGasTipCap() (*big.Int, error) {
	gasTip, err := Polygon.SuggestGasTipCap(context.Background())
	if err != nil {
		return nil, fmt.Errorf("[Polygon.getGasTipCap] internal error: %w", err)
	}
...
func getEIP1559Gas() (*big.Int, *big.Int, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("[Polygon.getEIP1559Gas] internal error: %w", err)
	}

	head, err := Polygon.HeaderByNumber(context.Background(), nil)
	if err != nil {
		return nil, nil, fmt.Errorf("[Polygon.HeaderByNumber] internal error: %w", err)
	}
...
func testTransaction(tx *types.Transaction) error {
	...
	_, err = Polygon.EstimateGas(context.Background(), callMsg)
