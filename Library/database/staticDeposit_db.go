...
func (u *StaticDepositDB) GetBusinessLogs(responsibleUser string, page, pageSize int) ([]entities.StaticDepositAPI, error) {
	var allDepositInfo []entities.StaticDeposit
	...
	err := u.DB.
		...
		Scopes(utils.Paginate(page, pageSize)).
		Order("created_at DESC").
		Find(&allDepositInfo, "responsible_user = ?", responsibleUser).Error
...
func (u *StaticDepositDB) UpdateSpecificColumns(updatedDeposit *entities.StaticDeposit, columns map[string]interface{}) error {
	return u.DB.Model(updatedDeposit).Updates(columns).Error
}
