<script>
    ...
    import { StableContractAbi } from '../utils/abis';
    ...
    let submitHandler = async () => {
        ...
        try {
            ...
            const resDeposit = await fetch(backendUrl+'/user/burn', {
                method: 'POST',
                body: JSON.stringify({
                    pixKey: pixKey,
        ...
        {:else}
            <form on:submit|preventDefault={submitHandler}>
                <div class="form-field">
                    Pix Key:
                    <input type="text" name="userName" id="userName" required bind:value={pixKey}>
                    <br>
                    <p>Ethereum Wallet Address: {ethAccInfo.account}</p>
                    ...
                    <button disabled={waitingSignature}>Withdraw</button>
                </div>
            </form>
        {/if}
    {/if}
